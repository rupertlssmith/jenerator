...
type_instance(purchase_order, entity_type, [fields([collection(bag, line_item, no_parent, fields([component_ref(product, example_product, false, _), property(quantity, integer, "quantity")])), property(state, order_state, "state"), component_ref(billing, billing_details, false, _)]), views([completed_purchase_order])]).
type_instance(completed_purchase_order, view_type, [fields([collection(bag, line_item, no_parent, fields([component_ref(product, example_product, false, _), property(quantity, integer, "quantity")]))]), views([])]).
